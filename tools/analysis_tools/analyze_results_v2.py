...
class ResultVisualizer:
    ...
    def evaluate_and_show(self,
        ...
        if isinstance(results1[0], tuple):
            results1 = [result[0] for result in results1]
        mAP1 = self.detection_evaluate(dataset, results1)

        if isinstance(results2[0], tuple):
            results2 = [result[0] for result in results2]
        mAP2 = self.detection_evaluate(dataset, results2)
        
        mAP = {}
        for idx in mAP1.keys():
            mAP[idx] = mAP1[idx] - mAP2[idx]
        
        mAP = list(sorted(mAP.items(), key=lambda kv: kv[1]))
        good_samples = mAP[-topk:]
        bad_samples = mAP[:topk]

        dir1 = osp.abspath(osp.join(show_dir, 'dir1'))
        dir2 = osp.abspath(osp.join(show_dir, 'dir2'))
        self._save_image_gts_results(dataset, results1, good_samples, dir1)
        self._save_image_gts_results(dataset, results2, good_samples, dir2)


    def detection_evaluate(self, dataset, results):
...
def parse_args():
    ...
    parser.add_argument(
        'prediction_path1', help='prediction path where test pkl result')
    parser.add_argument(
        'prediction_path2', help='prediction path where test pkl result')
    parser.add_argument(
...
def main():
    ...
    mmcv.check_file_exist(args.prediction_path1)
    mmcv.check_file_exist(args.prediction_path2)
    ...
    outputs1 = mmcv.load(args.prediction_path1)
    outputs2 = mmcv.load(args.prediction_path2)
    ...
    result_visualizer.evaluate_and_show(
        dataset, outputs1, outputs2, topk=args.topk, show_dir=args.show_dir)
