...
from .sph2pob_transform import Sph2PobTransfrom, jiter_spherical_bboxes
...
@weighted_loss
def normalized_l1_loss(y_true, y_pred, eps=1e-7):
    """
    ...
    pred = torch.stack([y_pred[:, 0]/(2*pi), y_pred[:, 1]/pi, torch.sigmoid(y_pred[:, 2]), torch.sigmoid(y_pred[:, 3])], dim=1)
    true = torch.stack([y_true[:, 0]/(2*pi), y_true[:, 1]/pi, torch.sigmoid(y_true[:, 2]), torch.sigmoid(y_true[:, 3])], dim=1)

    # Compute L1 loss for the stacked tensors
    abs_diff = torch.abs(pred - true)
    
    # Sum the absolute differences along the last dimension (box coordinates)
    l1_loss = torch.sum(abs_diff, dim=-1)
