...
def main():
    args = parse_args()
    if args.box_version in [4, 5]:
        import sys
        sys.path.append('/home/liuxinyuan/sph2obb')
        from sphdet.visualizers import imshow_det_bboxes
    else:
        from mmdet.core.visualization import imshow_det_bboxes
    ...
    for item in dataset:
        ...
        extra_args = dict(box_version=args.box_version) if args.box_version in [4, 5] else dict()
        imshow_det_bboxes(
            item['img'],
            gt_bboxes,
            gt_labels,
